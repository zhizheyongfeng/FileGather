...
使用企业微信、微信等工具下载的文件常常分布在不同的目录和层级里，换电脑时容易丢失。FileGather 可以按文件扩展名和文件名关键词检索指定目录（含子目录）下的文件，然后统一复制或移动到目标文件夹，方便整理和带走。

> **另有 Android 手机版**：同样的逻辑，在手机上把散落的文件收集起来。源码在仓库的 [FileGather.Android](FileGather.Android/) 目录，打包教程见 [BUILD_ON_ANDROID.md](FileGather.Android/BUILD_ON_ANDROID.md)。
...
- **增量同步**：重复同步时，目标已存在内容相同的文件自动跳过，不再重复复制（内容校验，可关闭）
- **实时进度**：执行时显示进度条和完成统计（成功 / 跳过 / 失败）
- **跟随系统深色模式**：界面配色自动适配系统的明 / 暗主题
...
5. **选择目标文件夹**：收集文件存放的位置
6. 选择操作方式：**复制**（保留源文件）或 **移动**（移除源文件）。移动会删除源文件，执行前会弹窗二次确认
7. **增量同步**：默认勾选「跳过已存在的相同文件」——如果目标已有内容相同的文件（比如上次同步过的），本次会自动跳过，只处理真正的新文件。想强制重新复制可取消勾选
8. 点击 **开始复制** / **开始移动**，完成后查看底部统计（成功 / 跳过 / 失败）
